...
> 将简答题、选择题Excel数据集加入Prompt合并转为Json文件保存，用作P-Tuning的训练数据集。

## 提示词设计

### 文本问答

我们在提示词中主要设置了对回答的限制及其举例，通过提示词的约束和相关实例来保证回答的简洁性。
...
提示词中主要用于连接公告内容理解和选项，使问题和选项自然过渡，便于模型理解，并且在一定范围上限制了回答的内容。
...
> 采用P-Tuning方法，通过冻结原预训练模型大部分参数，只微调小部分参数，加入Prompt后可基于小样本数据集进行训练，用于不同下游任务。
